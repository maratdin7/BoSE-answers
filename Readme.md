...
- sprint planning meeting — до начала каждого спринта (на нем определяется что будет делаться в спринте)  
- ежедневно — тот самый **scrum** (встреча разработчиков) 
...
- backlog 
  ...
  - не тз не спецификация
- sprint backlog
  - список целей/задач, необходимых для реализации спринта
  - *"журнал пожеланий спринта"*
- burndown chart (диаграмма сгорания задач)
  - Есть эталонная линия как должны идти задачи и линия как задачи идут. Они долны совпасть
    - Если они совпадают значит в проекте все хорошо
    - Если они немного разошлись но в конце проекта снова соединились то нормально
    - Если они не совпадают все плохо
- scrum board
  - минимальный размер — 3 столбца: 
    - TODO — что нужно реализовать за спринт
    - in progress — что сейчас реализуется
    - Done — сделано
...
- команда каждый день встречается на scrum-встрече
...
- выполняются спринты (автономно) (спринт можно остановить, но если произошло что-то совсем непоправимое)
...
- **Позднее принятие решений** 
  - **Не прогнозы, а факты** Сложные структуры реализуем когда в действительности становится фактом то, что нужно делать архитектуру сложнее
- **Быстрая доставка** (Маленькие версии) 
- **Мотивация команды** 
  - Люди — не ресурс. Иными словами — максимальная вовлеченность команды в принятие решений. 
- **Интегрирование** 
...
* Требования (заказчика) бывают неясны
...
- **Приоритезация** — требования должны быть разделены на относительные классы приоритетов, показывающие **значимость** в системе
...
- **Модифицируемость** — требования модифицируемы, если они сформулирвоаны таким образом, что предполагают относительно простую их модификацию
...
* Сведения о тестировании *(- Почему? — Пусть заказчик идет лесом, мы можем удовлетворить **уровень протестированности** (покрытие тестов, но это уже не о тестах, а о **требованиях к качеству**), а не написать ему N модульных тестов)*
...
Разработка требований — также отдельный этап инженерии требований. Ее разделяют на два этапа
...
1. **Семинар** — (самое популярное) открытое совместное обсуждение стоящих задач:
    * Заказчик:
...
1. **Интервью** — (если заказчик имеет смутное представление о разрабатываемом продукте, позволяет понизить требования к его IT-квалификации) целенаправленный опрос-интервью:
...
1. **Создание прототипов** — лучше один раз показать, чем много раз переспрашивать. Процесс:

    * Обсуждение стоящих высокоуровневых задач
    * Исполнитель:
      1. создает прототип
      1. демонстрирует
      1. исправляет недостатки
      1. документирует финальный прототип
    * Заказчик:
...
1. **Исследование** — исследуем рынок и потребности пользователя

    * Исполнитель:
      1. получает от заказчика общее представление о продукте *("- Хочу калькулятор, который завоюет рынок")*
      1. исследует рынок и выявляет востребованные характеристики продукта
      1. Анализирует решения конкурентов
      1. Документирует актуальные свойства проектируемого продукта
    * Заказчик:
      * Предоставление общего видения продукта
      * Корректировка и утверждение предложения исполнителя

1. **Работа с фокус-группами** 
    1. Составляем фокус-группу из будущих пользователей и ориентируемся на них (что из самых важных функций должно быть и т.п.)
    1. Обсудив продукт с фокус-группой, формируем ТЗ
    1. Обсуждаем его с заказчиком
...
- Определение всех артефактов (файлы, модели, документы), которые нуждаются в модификации, если изменение будет принято
- Определение задач, необходимых для реализации изменения 
- Оценка усилий для завершения этих задач 
- Оценка нахождения этих задач на критическом пути проекта *(Самый длинный последовательный путь от начала проекта до его завершения)*
...
- Контроль версий требований 
  1. Создание начальных версий требований 
  1. Ведение истории изменений 
  1. Авторизованный доступ к изменениям требований

### Состояния требований

1. **Предложено** — Требование было выставлено авторизованным источником
1. **Одобрено** — Требование было проанализировано и одобрено для определенной версии
1. **Реализовано** — Код, реализующий требование, был написан
1. **Проверено** — Корректная функциональность данного требования была подтверждена версией продукта
1. **Удалено** — Ранее одобренное требование было исключено из базисного списка; Причина удаления – задокументирована
1. **Отклонено** — Предложенное требование – отклонено(после анализа); Причина отклонения – задокументирована
...
Матрица прослеживния требований
...
- **Сотрудники**
  - Разделение на роли — конкретное амплуа сотрудника в конкретном проекте в определенное время. В программных проектах обычно оперируют ролями, а не сотрудниками: 
    - Заказчик (customer)
...
- **Оборудование** 
  - Специализированное оборудование для разработки/исполнения/тестирования проекта 
- **Машинное время**
...
- **ПО** (ОС, среда разработки и т. д.)
  - ПО для разработки/исполнения/тестирования проекта
...
- Проверяет качество программного обеспечения (функциональность, надежность, эффективность). 
